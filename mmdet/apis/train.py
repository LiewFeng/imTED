...
def train_detector(model,
    ...
    if cfg.load_from:
        # The modification is only for few shot object detection
        # Since score of background is in the last position
        ...
        for k, v in state_dict.items():
            if k.startswith('roi_head.bbox_head.fc_cls'):
                random_v = random_state_dict[k]
                random_v[:60] = v[:60]  # forground 
                random_v[-1] = v[-1]    # background
                new_state_dict[k] = random_v
            else:
                new_state_dict[k] = v
        load_state_dict(model, new_state_dict, strict=False, logger=logger)
